	...
	Promise.coroutine(function*() {
		...
		if (response.success) {
			...
			prompt.get(
				...
				function(err, result) {
					fs.exists('project-scratch-def.json', function(exists) {
						if (exists) {
							fs.readFile('project-scratch-def.json', 'utf8', function readFileCallback(err, data) {
...
fs.exists('dataCollection.json', function(exists) {
	if (exists) {
		fs.writeFile('dataCollection.json', convertToJson(result), 'utf8', function readFileCallback(err, data) {});
	}
});
// Process Begins
var LoginURL = {
